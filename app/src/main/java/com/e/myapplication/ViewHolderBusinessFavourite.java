...
public class ViewHolderBusinessFavourite extends RecyclerView.Adapter<ViewHolderBusinessFavourite.myViewHolder> {
    ...
    public ViewHolderBusinessFavourite(Context context, ArrayList<Upload> upload, String phonenumber) {
    ...
    @NonNull
    @Override
    public ViewHolderBusinessFavourite.myViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolderBusinessFavourite.myViewHolder(LayoutInflater.from(context).inflate(R.layout.categories_item,parent,false));
    }

    @Override
    public void onBindViewHolder(@NonNull final ViewHolderBusinessFavourite.myViewHolder holder, final int position) {
        per.keepSynced(true);
        use.keepSynced(true);
        ...
        FirebaseDatabase.getInstance().getReference("Business_fav").child(phonenumber).orderByChild("mobile").equalTo(upload.get(position).getContact_number()).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.getChildrenCount() == 0)
                {
                    ...
                    holder.firmname.setVisibility(View.INVISIBLE);
                    holder.proprietor_name.setVisibility(View.INVISIBLE);
                    holder.conduct_number.setVisibility(View.INVISIBLE);
